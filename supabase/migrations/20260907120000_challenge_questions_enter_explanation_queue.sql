-- Challenge questions join the staff explanation queue.
--
-- This reverses a deliberate decision rather than fixing an oversight. The
-- queue was introduced as "unanswered standard-difficulty items" and kept
-- Challenge out twice over: a `lower(q.difficulty) in ('easy','medium','hard')`
-- test, and a regex rejecting any question whose source archive named
-- Challenge. When the tier became a stored difficulty on 2026-09-01 the regex
-- was dropped as redundant, precisely because the difficulty test already
-- excluded Challenge on its own. So Challenge questions have never reached an
-- editor, by design.
--
-- Scott has asked for the ones missing explanations to be worked through, so
-- the tier is now admitted. If the original reason was that Challenge
-- explanations want his own hand rather than a staff editor's, that judgement
-- is the one being changed here -- the write boundary below is what enforces
-- it, so this is the only place it lives.
--
-- Five functions share the gate, and opening only the queue would have been
-- worse than leaving it shut: an editor would see a Challenge question, write
-- the explanation, and have the save rejected as "not eligible". They move
-- together.
--
--   get_explanation_queue          the list itself
--   get_explanation_queue_count    the open badge
--   update_staff_explanation       the write boundary for the explanation
--   update_staff_question_content  the write boundary for wording fixes
--   get_explanation_editor_stats   so completed Challenge work is credited
--
-- Only the question_bank branch of each changes. Practice-test questions
-- (public.questions) were deliberately left three-valued when the tier landed,
-- so their branch keeps the original list and is untouched here.
--
-- Each function is copied verbatim from the migration that last defined it
-- (20260901180000_challenge_by_stored_difficulty_only.sql) with exactly one
-- line changed, following the convention that migration set. Signatures are
-- unchanged, so CREATE OR REPLACE keeps existing privileges. They are restated
-- at the end regardless, so this file stands on its own and the deployment
-- verification in supabase/tests keeps passing whatever order it runs in.

-- get_explanation_queue: Challenge admitted to the question-bank branch.
...
as $$
  with bank_candidates as (
    ...
    where catalog.enabled = true
      and lower(q.difficulty) in ('easy', 'medium', 'hard', 'challenge')
...
-- get_explanation_queue_count: Challenge admitted to the question-bank branch.
...
-- update_staff_explanation: Challenge admitted to the question-bank branch.
...
begin
  ...
  if p_target_type = 'question_bank' then
    ...
    if not found
      ...
      or target_difficulty not in ('easy', 'medium', 'hard', 'challenge')
    then
...
-- update_staff_question_content: Challenge admitted to the question-bank branch.
...
begin
  ...
  if p_target_type = 'question_bank' then
    ...
    if not found or cur.difficulty is null or cur.difficulty not in ('easy', 'medium', 'hard', 'challenge') then
...
-- get_explanation_editor_stats: Challenge admitted to the question-bank branch.
