-- claim_billing_checkout_intent checked "plan/cadence differs" before it checked
-- "the 'creating' lease has gone stale". That meant an abandoned reservation (Stripe
-- setup failed before a session was ever created, or the tab was simply closed) for
-- one plan blocked switching to a *different* plan for up to the full one-hour
-- checkout_expires_at window, instead of the intended five-minute lease -- so a
-- student who got stuck trying Core would then also get "A different Checkout
-- session is still open" trying Max, for up to an hour. Reordering so a stale
-- 'creating' lease is reclaimed first (for whichever plan is now being requested)
-- fixes that, while a lease that is still active -- meaning a Stripe redirect may
-- genuinely be in flight -- still correctly blocks switching plans.
