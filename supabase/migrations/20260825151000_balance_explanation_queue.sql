...
as $$
  with bank_candidates as (
    ...
    order by (nullif(trim(q.explanation), '') is not null), source_label, location
    limit (greatest(2, least(coalesce(p_limit, 250), 500)) + 1) / 2
  ),
  test_candidates as (
    select
      q.id,
      ...
      q.prompt,
      q.figure_url,
      coalesce((select jsonb_agg(jsonb_build_object('id', c.letter, 'text', c.text) order by c.letter) from public.choices c where c.question_id = q.id), '[]'::jsonb) as choices,
      case
        when q.type = 'grid' then coalesce(array_to_string(q.accepted_answers, ' or '), '')
        else coalesce((select c.letter || '. ' || c.text from public.choices c where c.question_id = q.id and c.letter = q.correct limit 1), '')
      end as correct_answer,
      coalesce(q.explanation, '') as explanation,
    ...
    order by (nullif(trim(q.explanation), '') is not null), source_label, location
    limit greatest(2, least(coalesce(p_limit, 250), 500)) / 2
  ),
  candidates as (
  ...
  order by (nullif(trim(explanation), '') is not null), target_type, source_label, location
