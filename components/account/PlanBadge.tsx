...
export function PlanBadge({ plan, suspended = false, test = false }: { plan: PlanCode; suspended?: boolean; test?: boolean }) {
  if (suspended) {
    return <span className="inline-flex items-center rounded-full border border-danger/25 bg-danger-bg px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-[0.1em] text-danger-600">Suspended</span>;
  }
  return (
