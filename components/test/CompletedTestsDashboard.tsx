...
export function CompletedTestsDashboard({ attempts, testTitles, variant = "default" }: Props) {
  ...
  return (
    <>
      <section className={isUltimate ? "overflow-hidden rounded-[18px] border border-navy/10 bg-gradient-to-br from-navy via-navy-700 to-brand-600 shadow-pop" : "border-b border-brand/25 bg-gradient-to-br from-ice via-mist to-white"}>
        <div className={isUltimate ? "px-6 py-8 sm:px-8 sm:py-9" : "mx-auto max-w-[1100px] px-5 py-9 sm:px-6 sm:py-11"}>
          ...
          <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className={`mt-2 max-w-xl text-sm leading-6 ${isUltimate ? "text-white/70" : "text-shell-600"}`}>Track your score trajectory, revisit detailed reports, and share your progress.</p>
            </div>
      ...
      <div className={isUltimate ? "py-6" : "mx-auto max-w-[1100px] px-5 py-7 sm:px-6 sm:py-10"}>
        ...
        {scored.length > 0 ? (
          <section className="mt-6 rounded-2xl border border-shell-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-wrap items-end justify-between gap-2">
              ...
              <p className="text-xs text-shell-500">{scored.length} completed {scored.length === 1 ? "test" : "tests"}</p>
            </div>
        ...
        <section className="mt-8">
          ...
          {newestFirst.length === 0 ? (
            <div className="mt-4 rounded-2xl border border-dashed border-brand/35 bg-ice/60 p-10 text-center">
              <h3 className="font-display text-xl font-extrabold text-navy">Your first score will appear here.</h3>
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-shell-600">Complete a full-length practice test to unlock score trends and detailed module analytics.</p>
              ...
              {newestFirst.map((attempt, index) => {
                ...
                const change =
                  typeof attempt.totalScore === "number" && typeof prior?.totalScore === "number"
                    ...
                    : null;
                return (
                  <article key={attempt.id} className="overflow-hidden rounded-2xl border border-shell-200 bg-white shadow-sm">
                    <div className="border-b border-brand/12 bg-ice/70 px-5 py-3">
                      ...
                      <p className="mt-0.5 text-xs text-shell-500">{dateFormatter.format(new Date(attempt.createdAt))}</p>
                    </div>
                    <div className="p-5">
                      <div className="flex items-end justify-between gap-4">
                        <div>
                          <p className="text-xs font-bold uppercase tracking-[0.12em] text-shell-400">Total score</p>
                          <div className="mt-1 flex items-baseline gap-2">
                            <p className="font-display text-4xl font-black text-navy">{attempt.totalScore ?? "-"}</p>
                            {change != null ? <span className={`text-xs font-extrabold ${change >= 0 ? "text-brand-600" : "text-shell-500"}`}>{scoreChange(change)}</span> : null}
                          </div>
                        </div>
                        <div className="grid grid-cols-2 divide-x divide-shell-200 text-center">
                      ...
                      <div className="mt-5 grid grid-cols-2 gap-2">
                        ...
                        <button type="button" onClick={() => setShareAttemptId(attempt.id)} className="min-h-11 cursor-pointer rounded-lg border border-shell-300 bg-white px-3 text-sm font-bold text-shell-700 transition-colors hover:border-brand-600 hover:text-brand-600">Share score</button>
                      </div>
                    </div>
                  </article>
                );
...
function KpiCard({
  ...
  return (
    <article className="rounded-2xl border border-shell-200 bg-white p-5 shadow-sm">
      <p className="text-xs font-bold uppercase tracking-[0.12em] text-shell-500">{label}</p>
      <p className={`mt-2 font-display text-3xl font-black ${accent ? "text-brand-600" : "text-navy"}`}>{value ?? "-"}</p>
      <p className="mt-1 text-xs text-shell-500">{detail}</p>
    </article>
  );
}
...
function SmallScore({ label, value }: { label: string; value: number | null }) {
  return (
    <div className="min-w-16 px-3">
      <p className="text-[10px] font-bold uppercase tracking-wide text-shell-400">{label}</p>
