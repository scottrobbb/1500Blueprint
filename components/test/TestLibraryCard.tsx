...
export function TestLibraryCard({
  ...
  return (
    <article className={`group flex h-full flex-col overflow-hidden rounded-xl border bg-white transition-[border-color,box-shadow] duration-200 ${locked ? "border-gold/25" : "border-navy/12 hover:border-brand/40 hover:shadow-pop"}`}>
      <TestPreview variant={index % 4} locked={locked} />
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-brand-600">Full adaptive SAT</p>
            <h3 className="mt-1.5 font-display text-[22px] font-semibold leading-tight tracking-[-0.03em] text-ink">{title}</h3>
          </div>
          <div className="flex flex-none flex-wrap justify-end gap-2">
            {bestScore != null ? <ScoreBadge score={bestScore} /> : null}
            {draft ? <span className="rounded-full border border-gold/25 bg-[#fffaf0] px-2.5 py-1 text-[10px] font-semibold text-[#70550b]">Draft</span> : null}
            {planLocked ? <LockedBadge plan={requiredPlan} /> : null}
          </div>
        </div>

        <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-medium text-navy/50">
        ...
        <div className="mt-auto pt-5">
          {locked ? (
            planLocked ? (
              <Link href="/pricing" className="inline-flex min-h-11 w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-gold/30 bg-[#fffaf0] px-4 text-sm font-semibold text-navy transition-colors hover:border-gold/55 hover:bg-[#fff7e1] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand">
                <LockIcon className="h-4 w-4 text-[#7a5900]" /> Unlock with {requiredPlan === "core" ? "Core" : "Max"}
              </Link>
            ) : (
              <div className="flex min-h-11 items-center justify-center rounded-lg bg-navy/[0.05] px-4 text-sm font-semibold text-navy/40">Coming soon</div>
            )
          ) : (
            <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
              <Link href={`/practice-test/${slug}?workspace=ultimate`} className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-lg bg-navy px-5 text-sm font-semibold text-white transition-colors hover:bg-brand-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand">
                {actionLabel} <ChevronRightIcon className="h-4 w-4" />
              </Link>
              <Link href={`/practice-test/${slug}/modules?workspace=ultimate`} className="inline-flex min-h-11 cursor-pointer items-center justify-center rounded-lg border border-navy/15 bg-white px-4 text-sm font-semibold text-navy transition-colors hover:border-brand/35 hover:text-brand-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand">
                Modules
              </Link>
            </div>
          )}
...
function TestPreview({ variant, locked }: { variant: number; locked: boolean }) {
  return (
    <div aria-hidden="true" className={`relative h-[205px] overflow-hidden border-b border-navy/10 p-5 sm:h-[225px] sm:p-6 ${locked ? "bg-haze" : "bg-ice"}`}>
      <div className={`mx-auto h-full max-w-[500px] overflow-hidden rounded-lg border bg-white shadow-pop ${locked ? "border-navy/10 opacity-80" : "border-brand/15"}`}>
      ...
      <span className="absolute bottom-2.5 right-4 rounded-full border border-brand/15 bg-white/90 px-2 py-1 text-[8px] font-semibold uppercase tracking-[0.1em] text-brand-600">Digital SAT</span>
    </div>
  );
}
...
function ScoreBadge({ score }: { score: number }) {
  return (
    <span className="rounded-full border border-success/20 bg-success-bg px-2.5 py-1 text-right text-[10px] font-semibold text-success-600">
