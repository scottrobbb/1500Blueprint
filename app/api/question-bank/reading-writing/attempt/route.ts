...
export async function POST(request: Request) {
  ...
  if (existing.error) {
    reportServerError("question_bank.reading_writing.retry_check_failed", existing.error, {
      provider: "supabase",
      route: "/api/question-bank/reading-writing/attempt",
      method: "POST",
    });
  ...
  if (graded) {
    try {
      ...
      reportServerError("question_bank.reading_writing.xp_award_failed", error, {
        provider: "supabase",
        route: "/api/question-bank/reading-writing/attempt",
        method: "POST",
      });
...
async function isAnswerRevealed(email: string, questionId: string, correct: boolean): Promise<boolean> {
  ...
  try {
    ...
    reportServerError("question_bank.reading_writing.reveal_check_failed", error, {
      provider: "supabase",
      route: "/api/question-bank/reading-writing/attempt",
      method: "POST",
    });
