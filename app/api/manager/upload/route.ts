...
const EXPLANATION_ASSET_PATH = /^explanations\/\d{4}\/[0-9a-f-]+\.[a-z0-9]+$/i;
...
export async function POST(request: NextRequest) {
  ...
  let body: { name?: string; type?: string; size?: number } | null;
  try {
  ...
  try {
    const rate = await consumeRateLimit("manager-signed-upload", session.email, { limit: 60, windowSeconds: 60 * 60 });
...
export async function GET(request: NextRequest) {
  ...
  if (!path || !EXPLANATION_ASSET_PATH.test(path)) {
