...
export default async function UltimateCoursesPage() {
  ...
  const courses = await listCoursesForStudent(session.email);
      ...
      {courses.length > 0 ? (
        <>
          <section className="mb-6 grid gap-3 sm:grid-cols-3">
            <Metric label="Available courses" value={String(courses.length)} />
          ...
          <section className="grid gap-5 md:grid-cols-2">
            {courses.map((course) => (
              <Link key={course.id} href={`/ultimate/courses/${course.slug}`} className="group overflow-hidden rounded-[20px] border border-navy/10 bg-white shadow-pop transition-all hover:-translate-y-0.5 hover:border-brand/35">
                <div className="relative min-h-44 overflow-hidden bg-[linear-gradient(125deg,#0b2a5b,#174b91_65%,#3fa9f5)] p-6 text-white">
                  ...
                  <div className="relative">
                    ...
                    <p className="mt-3 text-xs font-semibold text-white/60">{course.modules.length} modules · {course.totalLessons} lessons</p>
                  </div>
                </div>
                <div className="p-5">
                  ...
                  <span className="mt-4 inline-flex min-h-11 items-center text-sm font-extrabold text-brand-600">{course.progress > 0 ? "Continue course" : "Start course"} <span className="ml-2 transition-transform group-hover:translate-x-1">→</span></span>
                </div>
              </Link>
            ))}
          </section>
