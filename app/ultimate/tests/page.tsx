...
export default async function UltimateTestsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1120px] px-4 pb-10 pt-8 sm:px-6">
      ...
      {access.plan === "free" ? (
        <UpgradePrompt currentPlan="free" requiredPlan="max" title="Max unlocks the full test library" description="Use the free test as a baseline, then compare it against 6 more adaptive scores with unlimited daily practice." features={["7 full-length tests", "Unlimited daily drills", "Challenge questions"]} className="mt-8" />
      ) : access.plan === "core" ? (
        <UpgradePrompt currentPlan="core" requiredPlan="max" title="Max adds the full test library" description="Max includes all 7 tests, the complete course library, and a planner that uses your score reports." features={["7 full-length tests", "All advanced courses", "Personal study planner"]} className="mt-8" />
      ) : null}
