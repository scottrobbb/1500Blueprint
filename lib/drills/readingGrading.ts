...
export const READING_GRADING_SYSTEM_PROMPT = [
  ...
  "Speech-to-text mangles decimals. 'Point three percent' arrives as '3 percent', 'zero point five' as '5'. A figure below one is the most likely thing in any summary to be transcribed wrong, and the student cannot see or correct the transcript.",
  "So weigh a figure against the claim the student states around it. When the words carry the relationship the point is about — far below the prediction, roughly double, barely moved — the point is recalled in full, even if the transcribed digits disagree with it. Only treat a figure as misremembered when the student's own surrounding words agree with the wrong figure.",
