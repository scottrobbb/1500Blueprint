...
const LESSON_COLUMNS = "id,month_id,title,vimeo_url,status,created_at,updated_at";
...
export async function listRecordingLibraryForAdmin(): Promise<CallRecordingMonth[]> {
  const [months, lessons] = await Promise.all([
    ...
    supabaseAdmin().from("call_recording_lessons").select(LESSON_COLUMNS).order("created_at").returns<LessonRow[]>(),
  ]);
...
export async function createRecordingLesson(input: CallRecordingLessonInput): Promise<CallRecordingLesson> {
  const { data, error } = await supabaseAdmin()
    ...
    .insert({ id: crypto.randomUUID(), month_id: input.monthId, title: input.title, vimeo_url: input.vimeoUrl, status: input.status })
...
export async function updateRecordingLesson(id: string, input: CallRecordingLessonInput): Promise<CallRecordingLesson> {
  const { data, error } = await supabaseAdmin()
    .from("call_recording_lessons")
    .update({ month_id: input.monthId, title: input.title, vimeo_url: input.vimeoUrl, status: input.status, updated_at: new Date().toISOString() })
    .eq("id", id)
    .select(LESSON_COLUMNS)
